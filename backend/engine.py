...
def build_credits(
    ...
    organization_costs_total = sum(float(value) for value in (balance_costs_by_day or costs_by_day).values())
    ...
    if balance is None and total_funds is not None:
        balance = total_funds - organization_costs_total

    # Project economics must always come from the official project Costs API.
    # The cabinet credit balance is organization-wide and cannot be used to
    # reverse-calculate project spend: credits may expire or be spent elsewhere.
    cost_per_ready = project_costs_total / ready_total if ready_total else None
    ...
    return {
        ...
        "costs_total_usd": round(project_costs_total, 2),
        "project_costs_total_usd": round(project_costs_total, 2),
        "organization_costs_total_usd": round(organization_costs_total, 2),
